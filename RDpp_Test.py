...
class TestDatasetWithLabel(Dataset):
    def __init__(self, folder):
        # good = glob(folder + "\\good\\*.jpg")  # 烟厂数据集
        # defection = glob(folder + "\\11\\*.jpg")  # 烟厂数据集
        # files_ori = glob(folder + "\\*\\*.png")  # 牙刷数据集
        self.files = glob(folder + "\\*\\*.jpg")  # 烟厂数据集

        seed(10)
        # shuffle(good)
        # shuffle(defection)
        # self.files = good[:1] + defection
        shuffle(self.files)
...
def test_aucroc(encoder, bn, decoder, proj, test_dl, mode="aucroc"):
    scores = []
    labels = []
    encoder.eval()
    bn.eval()
    ...
    if mode == "precise":
        ...
        return np.max(accuracies), scores_reserved, labels
...
def visualization(encoder, bn, decoder, proj, test_dl, mode="aucroc", _files=None):
    scores = []
    labels = []
    encoder.eval()
    bn.eval()
...
# state_dict = torch.load("rd_model_0809_100_12.pth")
...
# #     acc, scores, labels = test_aucroc(encoder, bn, decoder, proj_layer, test_dl, "aucroc")
# #     accs.append(acc)
# # print(round(np.mean(accs), 4))
# test_dataset = TestDatasetWithLabel(test_folder)
# test_dl = DataLoader(test_dataset, batch_size=1, shuffle=False)
# # acc, scores, labels = test_aucroc(encoder, bn, decoder, proj_layer, test_dl, "aucroc")
# # print(acc)
# # save_csv(scores, labels, test_dataset.files, "model_12_analyse.csv")
# visualization(encoder, bn, decoder, proj_layer, test_dl, "aucroc", test_dataset.files)
